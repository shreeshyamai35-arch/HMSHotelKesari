...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    const totalRoomsSetting = (await getSettingNumber(SETTING_TOTAL_ROOMS)) ?? 0;

    const todayRevenue = await prisma.revenueRecord.findFirst({ where: { recordDate: dayStart } });

    // Bookings that touch today, to derive check-ins/outs and in-house rooms.
    const relevant = await prisma.booking.findMany({
      where: {
    ...
    for (const b of relevant) {
      ...
      if (dateKey(arr) === dateKey(dayStart)) checkIns += b.roomsBooked;
      if (dateKey(dep) === dateKey(dayStart)) checkOuts += b.roomsBooked;
      if (arr.getTime() <= dayStart.getTime() && dep.getTime() > dayStart.getTime()) inHouse += b.roomsBooked;
    }
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    async function summarize(gte: Date, lte?: Date) {
      const where: Record<string, unknown> = { status: { not: 'CANCELLED' } };
      where.createdAt = { gte, ...(lte ? { lte } : {}) };
      const rows = await prisma.booking.findMany({ where });
      return {
    ...
    const [yesterday, last7, last30, currentMonth] = await Promise.all([
      ...
      summarize(addDaysUTC(now, -7), endOfDay(now)),
      summarize(addDaysUTC(now, -30), endOfDay(now)),
      summarize(monthStart, endOfDay(now)),
    ]);
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    for (const b of bookings) {
      ...
      const lead = Math.max(
        0,
        Math.round((startOfDay(b.arrivalDate).getTime() - startOfDay(b.bookingDate).getTime()) / 86400000)
      );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const otaByDay: Record<string, { ota: number; total: number }> = {};
    for (const b of bookings) {
      if (!b.arrivalDate) continue;
      const k = dateKey(startOfDay(b.arrivalDate));
      otaByDay[k] = otaByDay[k] ?? { ota: 0, total: 0 };
      otaByDay[k].total += b.amount;
      if (b.source === 'OTA') otaByDay[k].ota += b.amount;
    }
    ...
    const overrideLow = await getSettingNumber(SETTING_REVENUE_TIER_LOW);
    const overrideHigh = await getSettingNumber(SETTING_REVENUE_TIER_HIGH);
    const lowT = overrideLow ?? avg * 0.75;
    const highT = overrideHigh ?? avg * 1.25;

    function tier(v: number): 'HIGH' | 'MEDIUM' | 'LOW' | 'NONE' {
      if (v <= 0) return 'NONE';
      if (v >= highT) return 'HIGH';
      if (v < lowT) return 'LOW';
      return 'MEDIUM';
    }

    const byDate = new Map(records.map((r) => [dateKey(startOfDay(r.recordDate)), r]));
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push({
        ...
        occupancy:
          rec && rec.roomsAvailable > 0 ? +((rec.roomsSold / rec.roomsAvailable) * 100).toFixed(1) : 0,
        adr: rec ? rec.adr : 0,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const now = new Date();
    const year = /^\d{4}$/.test((req.query.year as string) || '') ? Number(req.query.year) : now.getFullYear();

    async function yearStats(y: number) {
      const start = startOfDay(new Date(y, 0, 1));
      const end = endOfDay(new Date(y, 11, 31));
      const records = await prisma.revenueRecord.findMany({ where: { recordDate: { gte: start, lte: end } } });
      const reviews = await prisma.review.findMany({ where: { reviewedAt: { gte: start, lte: end } } });
      ...
      for (const r of records) monthly[r.recordDate.getMonth()] += r.revenue;
