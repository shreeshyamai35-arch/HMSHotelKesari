...
function mockInsights(input) {
    ...
    return {
        summary: `Today the property recorded ${input.reportsToday} operations report(s). Revenue totals ₹${input.totalRevenue.toFixed(0)} with ${input.occupancy.toFixed(1)}% occupancy and an average guest rating of ${input.avgRating.toFixed(1)}/5.`,
        highlights,
        recommendations,
        provider: 'mock',
    };
}
async function openAiInsights(input) {
    // Pluggable real provider. Falls back to mock if anything fails.
    try {
        const prompt = `You are a hotel operations analyst. Given these metrics, return a JSON object with keys summary (string), highlights (string[]), recommendations (string[]). Metrics: ${JSON.stringify(input)}`;
