...
export async function generateOccupancyPDF(report: OccupancyReportData): Promise<Buffer> {
...
function renderSingleSlot(doc: PDFKit.PDFDocument, data: OccupancyReportData['data']) {
  const y = doc.y;
  ...
  if (data.submittedBy && data.submittedAt) {
    doc.moveDown(1);
    doc.fontSize(8).font('Helvetica').text(
      `Submitted by ${data.submittedBy} on ${formatDateTime(data.submittedAt)}`,
      50,
      doc.y,
      { align: 'left' }
    );
...
function renderDailyReport(doc: PDFKit.PDFDocument, data: OccupancyReportData['data']) {
  ...
  data.slots.forEach((slot, index) => {
    ...
    doc.fontSize(7).font('Helvetica').text(
      `Submitted by ${slot.submittedBy} on ${formatDateTime(slot.submittedAt)}`,
      50,
      doc.y + 5
    );
  });
...
function renderPeriodReport(doc: PDFKit.PDFDocument, data: OccupancyReportData['data']) {
